...
test.describe("Yaksha", () => {
   ...
   */
  test.only("TC1 - Verify My Info tab Loads", async () => {
 ...
 */
  test.only("TC3 -Verify the name gets edited successfully", async () => {
 ...
 */
test("TS_11Verify the Immigrant assign immigration record Add Functionality", async ({ page }) => {
 ...
 */
test.only("TS-13 Verify the Immigrant assign immigration record Delete Functionality", async ({ page }) => {
  const number = `PPN_${Date.now()}`;
...
test("TC-15 Verify the My Info's Qualification tab Loads Succesfully", async () => {                            // Step 1: Unique comment
  ...
  expect(list).toContain("https://opensource-demo.orangehrmlive.com/web/index.php/pim/viewQualifications/empNumber/");                                     // Ensure correct comment is present
});

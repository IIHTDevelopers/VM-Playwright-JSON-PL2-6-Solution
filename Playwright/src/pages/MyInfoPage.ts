 ...
 */
  ...
  async updateUniqueNAmeAndVerifyName(newName: string){
   ...
   return await this.page.locator("//p[@class='oxd-userdropdown-name']").innerText();
 ...
 */
  ...
  async addMembership(amount : string) {
    ...
    await this.page.waitForTimeout(6000);
    await this.page.locator(`//div[contains(@class, 'oxd-table-card') and contains(., '${amount}')]`).waitFor({ state: 'visible' });
    return this.page.locator("//div[contains(@class, 'orangehrm-container')]").nth(0).locator("//div/div/div/div[4]").allTextContents();
  }
 ...
 */
  async deleteMembership(amount: string) {
    ...
    await this.page.locator(`//div[@class='oxd-table-card'][.//div[text()='${amount}']]//i[@class='oxd-icon bi-trash']`).click();
    await this.page.waitForTimeout(1000);
 ...
 */
  async addImegration(PPn: string) {
    ...
    await this.page.waitForTimeout(3000);
    await this.page.locator(`//div[contains(@class, 'oxd-table-card') and contains(., '${PPn}')]`).waitFor({ state: 'visible' });
    return this.page.locator("//div[contains(@class, 'oxd-table')]").nth(0).locator("//div/div[3]").allInnerTexts();
    // // Return all passport numbers
 ...
 */
  async editImmigration(Uppn : string) {
    ...
    await this.page.locator(`//div[contains(@class, 'oxd-table-card') and contains(., '${Uppn}')]`).waitFor({ state: 'visible' });
    return this.page.locator("//div[contains(@class, 'oxd-table')]").nth(0).locator("//div/div[3]").allInnerTexts();
 ...
 */
  async deleteImmegration(PPn: string) {
    ...
    await this.psprtNumber.nth(1).fill(PPn);
    await this.saveBtn.click();
    await this.page.locator(`//div[contains(@class,'oxd-table-card') and contains(.,'${PPn}')]//i[contains(@class,'bi-trash')]`).waitFor({ state: 'visible' });
    await this.page.locator(`//div[contains(@class,'oxd-table-card') and contains(.,'${PPn}')]//i[contains(@class,'bi-trash')]`).click();
    await this.page.waitForTimeout(1000);
    await this.deletecnfrm.click();
    await this.page.locator("//div[@class='oxd-table-row oxd-table-row--with-border']//div[3]").waitFor({ state: 'visible' });
    await this.page.waitForTimeout(4000);
    return this.page.locator("//div[@class='oxd-table-row oxd-table-row--with-border']//div[3]").allInnerTexts(); // Return all passport numbers after deletion
